        ...
        for (int i = 0; i < lineCount; i++)
        {
            if (startTable)
            {
                ...
                else
                {
                    ...
                    {
                        //去掉格式<b><i><size=1>
                        temp[j] = temp[j].Replace("<b>", " ").Replace("</b>", " ").Replace("<i>", " ").Replace("</i>", " ").Replace("<size", " ").Replace("</size>", " ");
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            if (i >= tableStartRowID && i < tableEndRowID)
            {
                ...
                for (int j = lines[i].StartVertexIndex + 6; j <= lines[i].EndVertexIndex; j++)
                {
                    ...
                    vt.position = new Vector3(firstX + longTemp + rowID * _colSpacing + rowStartID * (_textSpacing), vt.position.y, vt.position.z) + new Vector3(posX, 0, 0);
                    if (vertexID == 5)
                    {
                        posX += longX + _charSpacing;
                    }
                    vertexs[j] = vt;
                    //以下注意点与索引的对应关系
                    if (j % 6 <= 2)
