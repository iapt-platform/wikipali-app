...
public /*final*/ class WesternDateKernel
{
    ...
    {
        return j2w(juliandate, (int)(calendarType), 0);
    }
    ...
    public static double w2j(int year, int month, int day, int calType, double SG)
    {
        ...
        double jd = day + Math.Floor((153 * month + 2) / 5.0) + (365.0 * year) + Math.Floor(year / 4.0);

        if (calType == 1)
        {
            jd = jd - Math.Floor((double)year / 100.0) + Math.Floor((double)year / 400.0) - 32045;
        }
        else if (calType == 2)
        {
            jd = jd - 32083;
        ...
        else
        {
            ...
            if (jd < SG)
            {
                jd = day + Math.Floor((double)(153.0 * month + 2.0) / 5.0) + (365.0 * year) + Math.Floor((double)year / 4.0) - 32083;
                if (jd > SG)
                    jd = SG;
            }
        }
    ...
    {
        return w2j(year, month, day, (int)calendarType, SG);
    }
    ...
    public static double w2j(int year, int month, int day, int hour, int minute, int second, CalendarType calendarType, double SG)
    {
        ...
        return w2j(year, month, day, (int)calendarType, SG) + fractionOfDay;
    }
    ...
    {
        return (int)w2j(year, month, 1, (int)Config.get().getCalendarType(), 0);
    }
    ...
    public static int getJulianDayNumberOfEndOfMonth(int year, int month)
    {
        ...
        int eml = getLengthOfMonth(year, month, (int)Config.get().getCalendarType());
        return js + eml - 1;
    }
